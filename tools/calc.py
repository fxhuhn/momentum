...
def aroon(high: pd.Series, low: pd.Series, lookback: int = 25) -> pd.Series:
    up = 100 * pd.Series(high).rolling(lookback).apply(lambda x: x.argmax()) / lookback
    dn = 100 * pd.Series(low).rolling(lookback).apply(lambda x: x.argmin()) / lookback
